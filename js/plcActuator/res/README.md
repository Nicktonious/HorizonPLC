...
Каналы актуаторов предоставляют набор механизмов, которые позволяют
...
Больше об обработке данных в соответствующем [разделе](../../plcChannel/res/README_DATA_REFINE.md#методы). 

Набор классов, обеспечивающих функционал датчика, можно условно поделить на следующие части: 
- Основная:
    - ветка классов [ClassBaseDevice](../../plcDevice/res/README.md#classbasedevice), [ClassDevice](../../plcDevice/res/README.md#classdevice) и [ClassActuator](./README_MIDDLE.md), хранящих в себе поля и методы, общие для всех актуаторов;
    - класс [ClassChannelActuator](README_CHANNEL.md), служит интерфейсом для работы с отдельным каналом актуатора;
- Сервисная: 
    - [ClassSuppression и ClassTransform](../../plcChannel/res/README_DATA_REFINE.md) реализует математико-логический аппарат для обработки и корректировки данных с датчика;
...
Рассмотрим основной набор параметров на примере конфига к [Buzzer](../../plcBuzzer/res/README.md). Детальное пояснение к параметрам описано в [классе-предке](../../plcDevice/res/README.md).
