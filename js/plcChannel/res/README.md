...
Если конфигурация канала содержит поле `valueType: "string"`, то поле `_IsNumType` приобретает значение `false`. При `this._IsNumType == false`, то значение **Value** канала считывается или сохраняется напрямую, в обход математической обработки. 
